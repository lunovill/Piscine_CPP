...
ClapTrap::ClapTrap(std::string name) : _name(name), _hitPoint(10), _energyPoint(10), _attackDamage(0) {
	std::cout << ansi((short[]){BOLD, ITALIC, GREEN}, 3) + "Constructor called." << std::endl;
	return;
}

ClapTrap::ClapTrap(const ClapTrap &copy) : _name(copy.getName()), _hitPoint(copy.getHitPoint()), _energyPoint(copy.getEnergyPoint()), _attackDamage(copy.getAttackDamage()) {return;}
...
ClapTrap::~ClapTrap(void) {
	std::cout << ansi((short[]){BOLD, ITALIC, RED}, 3) + "Destructor called." << std::endl;
	return;
}
...
void	ClapTrap::takeDamage(unsigned int amount) {
	if (!this->_hitPoint) {
		...
		std::cout << ansi((short[]){YELLOW, BOLD}, 2) + "ClapTrap " << std::endl;
...
void	ClapTrap::beRepaired(unsigned int amount) {
	if (!this->_hitPoint) {
		...
		this->_energyPoint--;
		std::cout << ansi((short[]){CYAN, BOLD}, 2) + "ClapTrap " << std::endl;
