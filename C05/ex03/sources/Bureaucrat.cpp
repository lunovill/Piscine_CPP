...
void	Bureaucrat::executeForm(const AForm &f) const {
	try {
		f.execute(*this);
		std::cout << ansi((short[]){ITALIC, CYAN}, 2) << this->_name << " executed " << f.getName() << "." << std::endl;
	} catch (AForm::GradeTooLowException &e) {
		std::cout << ansi((short[]){ITALIC, CYAN}, 2) << this->_name << " couldn't execute " << f.getName() << " because " << e.what() << "." << std::endl;
	} catch (AForm::FormNoSigned &e) {
		std::cout << ansi((short[]){ITALIC, CYAN}, 2) << this->_name << " couldn't execute " << f.getName() << " because " << e.what() << "." << std::endl;
	}
	return;
}
